...
if __name__=="__main__":
    ...
    for scenario in range(1):
	  ...
	  if scenario==0:
  	    width=avtotwidth(0,gDM,gq,mMed,mDM)/mMed
	  else:
  	    width=avtotwidth(2,gDM,gq,mMed,mDM)/mMed
	  if gqlimits[-1]<0 and width>widthlimit:
	    gqlimits[-1]=gq
            print gq, width, widthlimit
       pointsX=array.array("f",[float(mMed)/1000. for mMed in mMeds])
       pointsY=array.array("f",[float(gqlimit) for gqlimit in gqlimits])
    ...
    mg.Draw("alp")
    mg.SetTitle("")
    mg.GetXaxis().SetTitle("mediator mass [TeV]")
    mg.GetYaxis().SetTitle("g_{q}")
    mg.GetXaxis().SetRangeUser(0,8)
    ...
    c.SaveAs("dm_width_limit.pdf")
    c.SaveAs("dm_width_limit.eps")
