...
if calcUncert:
    ...
    qcdmufile=TFile.Open("2jet.NNLO.fnl5662j_mjj_chi_"+PDF+"_cppread_mu_"+mscale+".root")
    qcdmemfile=TFile.Open("2jet.NNLO.fnl5662j_mjj_chi_"+PDF+"_cppread_mem_"+mscale+".root")
...
if compareUncert:
    ...
    for style in styles:
        for i in range(5,31):
            ciqcdfilename="fnl5662j_cs_ct14nlo_".replace("ct14nlo",PDF)+str(i*1000)+"_"+style+".root"
            ...
            for j in range(2,8):
                ...
                legend.AddEntry(hist1s[j-2],"2015 NLOJET","l")
                legend.AddEntry(hist2s[j],"Run2 NLOJET+CIJET","l")
            ...
            for j in range(2,8):
                ...
                legend.SetFillStyle(0)
                legend.AddEntry(hist1pdfups[j-2],"2015 NLOJET PDF Uncertainty","l")
                legend.AddEntry(hist1scaleups[j-2],"2015 NLOJET scale Uncertainty","l")
                legend.AddEntry(hist2pdfups[j],"Run2 NLOJET+CIJET PDF Uncertainty","l")
                legend.AddEntry(hist2scaleups[j],"Run2 NLOJET+CIJET scale Uncertainty","l")
            ...
            for j in range(2,8):
                ...
                hist1pdfups[j-2].SetMinimum(0.85)
                hist1pdfups[j-2].SetMaximum(1.5)
                hist1pdfups[j-2].GetXaxis().SetTitle("#chi_{dijet}")
                hist1pdfups[j-2].Draw("hist")
