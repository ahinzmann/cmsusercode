...
"SumInQuadrature" ]
	...
	maxy=max(cloneNormalize(datahist[j]).GetMaximum(),alt.GetMaximum())
      ...
      canvas.SaveAs(prefix + "_"+samples[i][0].replace("QCD","") + '_sys2017.pdf')
      canvas.SaveAs(prefix + "_"+samples[i][0].replace("QCD","") + '_sys2017.eps')
	...
	print "Applying response matrix"
	  ...
	  for j in range(len(massbins)):
        ...
        canvas.SaveAs(prefix + "_"+samples[i][0].replace("QCD","") + '_sys2017_smear.pdf')
        canvas.SaveAs(prefix + "_"+samples[i][0].replace("QCD","") + '_sys2017_smear.eps')
