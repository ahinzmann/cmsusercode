...
for massbins in massbinssets:
    ...
    f=TFile(prefix+"_"+str(signalf)+"_chi.root")
    cfg.writelines("""
...
""")
    for i in range(len(massbins)):
        cfg.writelines("""shapes * bin"""+str(i)+""" """+prefix+"""_"""+str(signalf)+"""_chi.root $PROCESS#chi"""+str(massbins[i][0])+"""_"""+str(massbins[i][1])+"""_rebin1 $PROCESS#chi"""+str(massbins[i][0])+"""_"""+str(massbins[i][1])+"""_rebin1_$SYSTEMATIC
""")
    cfg.writelines("""-----------
""")
    text="bin "
...
""")
    ...
    text+="\nprocess "
    for i in range(len(massbins)):
       text+=""+signalWithMass+" "+signalWithMass+"_ALT "
    text+="\nprocess "
    for i in range(len(massbins)):
       text+="0 1 "
    text+="\nrate "
...
""")
    ...
    whatfit=("" if withSignal else "--fixedSignalStrength=0")
    os.system("combine chi_datacard_"+name+"_bestfit_run2.txt -M GoodnessOfFit --algo saturated "+whatfit+" -n "+name+"goodnessfit |& tee goodnessfit"+name+".txt")
    for toy in range(0,20):
       command="combine chi_datacard_"+name+"_bestfit_run2.txt -M GoodnessOfFit --algo saturated "+whatfit+" -t 450 --saveToys -n "+name+"goodnessfittoys_toy"+str(toy)+" |& tee goodnessfittoys"+name+"_toy"+str(toy)+".txt"
